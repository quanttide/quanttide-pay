...
| `packages/py/` | Python 实现（计划中） | ⏸ 未启（G6 暂缓，见下） |
| `tests/fixtures/` | 共享契约测试向量（JSON，契约唯一权威） | ✅ 3 份（money/status/ledger），billing 待补 |
| `pyproject.toml` | Python 包配置留位（version 0.0.1，pytest 配置） | ⏸ 留位，无 `src/` 实现 |
...
| G6 | P2 | Python 实现对齐：`packages/py` 消费同一 fixtures（先实现 money/status/order 号，再扩 billing/ledger）；补 pytest runner | `packages/py/` + `pyproject.toml` 启用 | 暂缓（fixtures/runner 结构已留位，需要时再启） |

## 不做

- ~~多语言强制对齐~~：G6 暂缓，不设各语言实现时间线；契约演进由 fixtures 驱动，各端自行跟进
